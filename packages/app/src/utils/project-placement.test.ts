...
describe("project-placement", () => {
  ...
  it("normalizes Doya worktree paths into the parent repo key", () => {
    const placement = deriveProjectPlacementFromCwd("/Users/test/repo/.doya/worktrees/feature-x");

    expect(placement.projectKey).toBe("/Users/test/repo");
    expect(placement.projectName).toBe("repo");
    expect(placement.checkout.cwd).toBe("/Users/test/repo/.doya/worktrees/feature-x");
  });
  ...
  it("prefers an existing placement when present", () => {
    const existing = {
      ...
      checkout: {
        ...
        isDoyaOwnedWorktree: false as const,
        mainRepoRoot: null,
      },
    };
