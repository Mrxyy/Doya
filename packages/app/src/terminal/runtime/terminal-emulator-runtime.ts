...
} from "@getdoya/protocol/terminal-input-mode";
import {
...
export class TerminalEmulatorRuntime {
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    window.__doyaTerminal = terminal;
  ...
  unmount(): void {
    ...
    this.cleanup?.();
    this.cleanup = null;
    if (window.__doyaTerminal === this.terminal) {
      window.__doyaTerminal = undefined;
    }
    this.terminal = null;
    this.fitAddon = null;
