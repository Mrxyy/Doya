...
export function buildComposerAiCreationPrompt(input: {
  ...
  if (input.context.mode === "slides") {
    return buildAiCreationMarkupPrompt({
      ...baseInput,
      ratio: input.context.ratio,
      aiInstructions: buildSlidesPrompt({ prompt, sourceFileCount: input.attachmentCount }),
    });
  }
...
function buildSlidesPrompt(input: { prompt: string; sourceFileCount: number }): string {
  return [
    "Create a presentation for this request:",
    input.prompt,
    "",
    "Use the existing AI creation / PPT creation workflow available in this workspace.",
    "Produce a live preview when possible and include the preview path in the result.",
