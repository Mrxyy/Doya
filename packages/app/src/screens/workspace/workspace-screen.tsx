...
function isWorkspacePreviewTarget(target: WorkspaceTabTarget): boolean {
  return target.kind === "pptPreview" || target.kind === "file";
}

function isWorkspacePreviewTabId(tabId: string): boolean {
  return tabId.startsWith("ppt_preview_") || tabId.startsWith("file_");
}
...
function WorkspaceScreenContent({
  ...
  useEffect(() => {
    ...
    const tabsById = new Map(collectAllTabs(workspaceLayout.root).map((tab) => [tab.tabId, tab]));
    const isPreviewTab = (tabId: string): boolean => {
      const tab = tabsById.get(tabId);
      return Boolean(
        tab && (isWorkspacePreviewTarget(tab.target) || isWorkspacePreviewTabId(tabId)),
      );
    };
    ...
    for (const [previewTabId, parentTabId] of Object.entries(
      workspaceLayout.parentTabIdByTabId ?? {},
    )) {
      const previewTab = tabsById.get(previewTabId);
      if (
        !previewTab ||
        (!isWorkspacePreviewTarget(previewTab.target) && !isWorkspacePreviewTabId(previewTabId))
      ) {
        continue;
      }
      const parentPane = findPaneContainingTab(workspaceLayout.root, parentTabId);
      const previewPane = findPaneContainingTab(workspaceLayout.root, previewTabId);
      if (!parentPane || !previewPane || parentPane.id !== previewPane.id) {
        continue;
      }

      restorePreviewSplit({ sourcePaneId: parentPane.id, previewTabId });
      return;
    }

    for (const pane of collectAllPanes(workspaceLayout.root)) {
      if (pane.tabIds.length < 2) {
        continue;
      }
      const previewTabId = pane.tabIds.find((tabId) => isPreviewTab(tabId));
      if (!previewTabId) {
        continue;
      }
      const sourceTabId = pane.tabIds.find((tabId) => tabId !== previewTabId);
      if (!sourceTabId) {
        continue;
      }

      restorePreviewSplit({ sourcePaneId: pane.id, previewTabId });
      return;
    }
  }, [
    canRenderDesktopPaneSplits,
