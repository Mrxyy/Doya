...
const test = base.extend<{ doyaE2ESetup: void; withWorkspace: WithWorkspace }>({
    ...
    async ({ page }, provide, testInfo) => {
      ...
      await page.addInitScript(
        ({ daemon, preferences, seedNonce: nonce }) => {
          ...
          localStorage.setItem("@doya:daemon-registry", JSON.stringify([daemon]));
          localStorage.removeItem("@doya:settings");
          localStorage.setItem("@doya:create-agent-preferences", JSON.stringify(preferences));
        },
