 ...
 * status endpoint, PID from the doya.pid lock file, log path from the
 * E2E_DOYA_HOME directory. Call this in Node test code (not in the browser).
 */
export async function loadRealDaemonState(): Promise<RealDaemonState> {
  const port = getE2EDaemonPort();
  const doyaHome = process.env.E2E_DOYA_HOME;
  if (!doyaHome) throw new Error("E2E_DOYA_HOME not set — globalSetup must run first");
  ...
  try {
    const raw = readFileSync(`${doyaHome}/doya.pid`, "utf8");
    ...
    console.warn("[desktop-updates] doya.pid not found:", err);
  }

  return { version: data.version, pid, logPath: `${doyaHome}/daemon.log` };
}
...
/**
 * Injects window.doyaDesktop before app load so all Electron-gated code
...
export async function injectDesktopBridge(page: Page, config: DesktopBridgeConfig): Promise<void> {
  await page.addInitScript((cfg) => {
    ...
    (window as unknown as { doyaDesktop: unknown }).doyaDesktop = {
      platform: "darwin",
