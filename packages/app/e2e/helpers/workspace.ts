...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@doya.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Doya E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.doyaConfig) {
    await writeFile(path.join(repoPath, "doya.json"), JSON.stringify(options.doyaConfig, null, 2));
  }
  ...
  if (options?.doyaConfig) {
    execSync("git add doya.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.doya-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "doya-e2e-dir-"): Promise<TempDirectory> {
