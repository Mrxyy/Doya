...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@doya.test"], localPath);
  git(["config", "user.name", "Doya E2E"], localPath);
...
// Bulk cleanup relies on this prefix being unmistakable — never reuse `doya-`
// (collides with real repos like `doya`, `doya-website`).
const TEMP_GITHUB_REPO_PREFIX = "doyatmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@doya.test"], basePath);
  git(["config", "user.name", "Doya E2E"], basePath);
