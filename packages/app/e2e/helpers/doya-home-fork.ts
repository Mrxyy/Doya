...
export interface DoyaHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveDoyaHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkDoyaHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<DoyaHomeMetadataForkResult> {
  const sourceHome = resolveDoyaHomePath(input.sourceHome);
  const targetHome = resolveDoyaHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Doya metadata onto the same DOYA_HOME.");
  }
...
export type DoyaHomeMetadataForkResultLegacy = DoyaHomeMetadataForkResult;
export const resolveDoyaHomePathLegacy = resolveDoyaHomePath;
export const forkDoyaHomeMetadataLegacy = forkDoyaHomeMetadata;
