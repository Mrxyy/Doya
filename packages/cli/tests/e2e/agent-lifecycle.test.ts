 ...
 * 2. Create agent with `doya run "say hello" --provider claude`
 * 3. List agents with `doya ls`
 * 4. Wait for agent with `doya wait <id>`
 * 5. Inspect agent with `doya inspect <id>`
 * 6. Stop agent with `doya stop <id>` and verify it remains inspectable
 * 7. Delete agent with `doya delete <id>`
...
async function setup(): Promise<void> {
  ...
  try {
    ...
    console.log(`DOYA_HOME: ${ctx.doyaHome}`);
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.doya(["ls", "--json"]);
...
async function test_agent_wait(agentId: string): Promise<void> {
  ...
  const result = await ctx.doya(["wait", "--timeout", "120s", agentId], { timeout: 130000 });
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.doya(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.doya(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.doya(["stop", agentId]);
  ...
  const inspectResult = await ctx.doya(["inspect", agentId]);
...
async function test_agent_delete(agentId: string): Promise<void> {
  ...
  const result = await ctx.doya(["delete", agentId]);
