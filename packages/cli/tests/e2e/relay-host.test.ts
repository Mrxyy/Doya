...
import { parseConnectionOfferFromUrl } from "@getdoya/protocol/connection-offer";
import { generateLocalPairingOffer } from "@getdoya/server";
import { DaemonClient } from "@getdoya/client/internal/daemon-client";
...
(shouldRunRelayE2e ? describe : describe.skip)("CLI --host offer URL via relay", () => {
  ...
  beforeAll(async () => {
    ...
    ctx = await createE2ETestContext({
      timeout: 60_000,
      env: {
        DOYA_RELAY_ENABLED: "true",
        DOYA_RELAY_ENDPOINT: relayEndpoint,
        DOYA_RELAY_PUBLIC_ENDPOINT: relayEndpoint,
      },
    });

    const offer = await generateLocalPairingOffer({
      doyaHome: ctx.doyaHome,
      relayEnabled: true,
      relayEndpoint,
  ...
  it("runs `doya --host <offer-url> ls` over the relay and matches direct ls output", async () => {
    if (!ctx) throw new Error("test context not initialized");

    const direct = await ctx.doya(["ls", "--json"]);
    ...
    const relay = await ctx.doya(["ls", "--json", "--host", offerUrl], {
      timeout: 30_000,
      env: { DOYA_HOST: offerUrl },
    });
