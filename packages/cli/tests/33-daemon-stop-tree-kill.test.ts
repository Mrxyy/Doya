...
const doyaHome = await mkdtemp(join(tmpdir(), "doya-stop-tree-kill-"));
const childPidPath = join(doyaHome, "descendant.pid");
...
try {
  ...
  const stopResult =
    await $`DOYA_HOME=${doyaHome} DOYA_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.DOYA_LOCAL_SPEECH_AUTO_DOWNLOAD} DOYA_DICTATION_ENABLED=${testEnv.DOYA_DICTATION_ENABLED} DOYA_VOICE_MODE_ENABLED=${testEnv.DOYA_VOICE_MODE_ENABLED} npx doya daemon stop --home ${doyaHome} --json --timeout 1 --force --kill-timeout 2`.nothrow();
