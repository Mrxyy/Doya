 ...
 *
 * Tests daemon commands with an isolated DOYA_HOME.
 *
 * Tests:
...
function daemonCommand(args: string[]) {
  return runLocalDoya(["daemon", ...args], { DOYA_HOME: doyaHome });
}
...
try {
  // Test 1: daemon --help shows subcommands
  {
    ...
    const result = await runLocalDoya(["daemon", "--help"]);
  ...
  // Test 5: daemon status --json outputs valid JSON
  {
    ...
    assert.strictEqual(status.home, doyaHome, "json status should reflect the isolated home");
    assert.strictEqual(
      status.hostname,
      null,
