 ...
 * Test Daemon Helper
 *
 * Provides utilities for launching real Doya daemons in E2E tests.
 * Each test gets an isolated daemon on an available local port with its own DOYA_HOME.
 *
...
const TEST_OUTPUT_CAPTURE_LIMIT = Number.parseInt(
  process.env.DOYA_TEST_OUTPUT_CAPTURE_BYTES ?? `${DEFAULT_OUTPUT_CAPTURE_LIMIT}`,
  10,
);
...
export async function createTempDirs(): Promise<{ doyaHome: string; workDir: string }> {
  const doyaHome = await mkdtemp(join(tmpdir(), "doya-e2e-home-"));
  const workDir = await mkdtemp(join(tmpdir(), "doya-e2e-work-"));

  // Create the agents directory that the daemon expects
  const agentsDir = join(doyaHome, "agents");
...
/**
 * Wait for daemon to be ready by running `doya agent ls`
...
async function probeDaemonReady(port: number): Promise<boolean> {
  try {
    const { exitCode } = await runDoyaCli(
      {
        port,
 ...
 * with isolated DOYA_HOME and DOYA_LISTEN environment variables.
 */
export async function startTestDaemon(options?: {
  ...
  const { doyaHome, workDir } =
    options?.doyaHome && options?.workDir
      ? { doyaHome: options.doyaHome, workDir: options.workDir }
  ...
  const daemonProcess = spawn("npx", ["tsx", cliSrcPath, "daemon", "start", "--foreground"], {
    env: {
      ...
      DOYA_HOME: doyaHome,
      DOYA_LISTEN: `${TEST_DAEMON_HOST}:${port}`,
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn("npx", ["tsx", cliSrcPath, ...args], {
      env: {
        ...process.env,
        ...TEST_DAEMON_ENV_DEFAULTS,
        DOYA_HOST: `${TEST_DAEMON_HOST}:${ctx.port}`,
        DOYA_HOME: ctx.doyaHome,
        ...options?.env,
      },
      cwd,
      stdio: ["ignore", "pipe", "pipe"],
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(new Error(`CLI command timed out after ${timeout}ms: doya ${args.join(" ")}`));
    }, timeout);
...
export async function createE2ETestContext(options?: {
  ...
  TestDaemonContext & {
    /** Run a Doya CLI command against this daemon */
    doya: (
      args: string[],
