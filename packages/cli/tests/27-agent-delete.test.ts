...
const doyaHome = await mkdtemp(join(tmpdir(), "doya-delete-test-home-"));
...
async function runDelete(args: string[]) {
  return $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npm --prefix ${repoRoot} run cli -- delete ${args}`.nothrow();
}
...
try {
  ...
  {
    console.log("Test 7: doya --help shows delete command");
    ...
    console.log("✓ doya --help shows delete command\n");
  }

  {
    console.log("Test 8: -q (quiet) flag is accepted with delete");
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npm --prefix ${repoRoot} run cli -- -q delete abc123`.nothrow();
    const output = result.stdout + result.stderr;
