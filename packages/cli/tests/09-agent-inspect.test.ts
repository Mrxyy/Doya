...
try {
  // Test 1: inspect --help shows options
  {
    ...
    const result = await $`npx doya inspect --help`.nothrow();
  ...
  // Test 2: inspect requires id argument
  {
    ...
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npx doya inspect`.nothrow();
  ...
  // Test 3: inspect handles daemon not running
  {
    ...
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npx doya inspect abc123`.nothrow();
  ...
  // Test 4: inspect --host flag is accepted
  {
    ...
    const result =
      await $`DOYA_HOME=${doyaHome} npx doya inspect --host localhost:${port} abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 5: -q (quiet) flag is accepted with inspect
  {
    ...
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npx doya -q inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 6: --json flag is accepted with inspect
  {
    ...
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npx doya inspect abc123 --json`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 7: --format yaml flag is accepted with inspect
  {
    ...
    const result =
      await $`DOYA_HOST=localhost:${port} DOYA_HOME=${doyaHome} npx doya --format yaml inspect abc123`.nothrow();
    const output = result.stdout + result.stderr;
  ...
  // Test 8: doya --help shows inspect command
  {
    console.log("Test 8: doya --help shows inspect command");
    ...
    console.log("doya --help shows inspect command\n");
  }
