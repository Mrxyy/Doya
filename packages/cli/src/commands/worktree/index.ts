...
export function createWorktreeCommand(): Command {
  const worktree = new Command("worktree").description("Manage Doya-managed git worktrees");

  addJsonAndDaemonHostOptions(
    worktree.command("ls").description("List Doya-managed git worktrees"),
  ).action(withOutput(runLsCommand));

  addJsonAndDaemonHostOptions(
    worktree
      .command("create")
      .description("Create a Doya-managed git worktree")
