...
import {
  ...
  createDoyaWorktree,
  type CreateDoyaWorktreeDeps,
} from "./doya-worktree-service.js";
import { readDoyaWorktreeMetadata } from "../utils/worktree-metadata.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createDoyaWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createDoyaWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createDoyaWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  const created = await createDoyaWorktree(
    {
      cwd: repoDir,
  ...
  expect(readDoyaWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "doya-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
interface TestDeps extends CreateDoyaWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isDoyaOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
