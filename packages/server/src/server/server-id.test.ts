...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "doya-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.DOYA_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per DOYA_HOME", () => {
  ...
  it("respects and persists DOYA_SERVER_ID override", () => {
    process.env.DOYA_SERVER_ID = "test-daemon-id";
