...
describe("generateBranchNameFromFirstAgentContext", () => {
  ...
  test("keeps the branch slug validator fallback when instructions are present", async () => {
    const repoRoot = createTempDir("doya-branch-config-");
    const worktreeRoot = createTempDir("doya-branch-worktree-");
    mkdirSync(path.join(worktreeRoot, ".git"));
    writeDoyaWorktreeMetadata(worktreeRoot, { baseRefName: "main" });
    writeDoyaWorktreeFirstAgentBranchAutoNameMetadata(worktreeRoot, {
