...
function createGitRepo(): string {
  ...
  execFileSync("git", ["config", "user.email", "test@getdoya.local"], {
    cwd: repoDir,
    stdio: "pipe",
  });
  execFileSync("git", ["config", "user.name", "Doya Test"], { cwd: repoDir, stdio: "pipe" });
...
async function expectWorktreeAbsentFromList(repoDir: string, worktreePath: string): Promise<void> {
  await expect
    .poll(
      async () => {
        const listed = await ctx.client.getDoyaWorktreeList({ cwd: repoDir });
...
test("create_agent_request creates a worktree and auto-archives both after the first turn", async () => {
  ...
  const listedWithWorktree = await ctx.client.getDoyaWorktreeList({ cwd: repoDir });
...
test("create_agent_request with autoArchive archives only the agent when no worktree was created", async () => {
  ...
  const worktrees = await ctx.client.getDoyaWorktreeList({ cwd: repoDir });
...
test("create_agent_request with worktree but no autoArchive leaves agent and worktree active", async () => {
  ...
  await ctx.client.archiveDoyaWorktree({ worktreePath: created.worktreePath });
});
