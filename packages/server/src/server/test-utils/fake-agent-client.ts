...
import { isLikelyExternalToolName } from "@getdoya/protocol/tool-name-normalization";
...
class FakeAgentSession implements AgentSession {
  ...
  private async runSlashCommand(
    ...
    if (this.providerName === "codex" && fullName.startsWith("prompts:")) {
      ...
      return {
        text: `DOYA_OK ${args ?? ""}`.trim(),
        timeline: [{ type: "assistant_message", text: `DOYA_OK ${promptId}` }],
        usage: { inputTokens: 1, outputTokens: 1 },
      };
    }
    ...
    return {
      text: "DOYA_SKILL_OK",
      timeline: [{ type: "assistant_message", text: "DOYA_SKILL_OK" }],
      usage: { inputTokens: 1, outputTokens: 1 },
    };
  }
