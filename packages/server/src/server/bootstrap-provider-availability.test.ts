...
describe("bootstrap provider availability", () => {
  ...
  test("loads a persisted Codex record without spawning a missing Codex binary", async () => {
    const { createDoyaDaemon } = await import("./bootstrap.js");
    const root = await mkdtemp(path.join(os.tmpdir(), "doya-bootstrap-provider-"));
    tempRoots.push(root);
    const binDir = await mkdtemp(path.join(os.tmpdir(), "doya-bootstrap-provider-bin-"));
    ...
    const agentStoragePath = path.join(doyaHome, "agents");
    ...
    const daemon = await createDoyaDaemon(config, pino({ level: "silent" }));
    try {
