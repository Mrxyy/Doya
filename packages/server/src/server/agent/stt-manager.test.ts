...
describe("STTManager", () => {
  function resolveVoiceLanguage(params: {
  ...
  it("does not force a language when no voice language config is set", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({});

    expect(fakeStt.lastLanguage).toBeUndefined();
  });

  it("uses DOYA_VOICE_LANGUAGE over DOYA_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        DOYA_VOICE_LANGUAGE: "pt",
        DOYA_DICTATION_LANGUAGE: "es",
  ...
  it("uses DOYA_DICTATION_LANGUAGE when DOYA_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        DOYA_DICTATION_LANGUAGE: "pt",
      } as NodeJS.ProcessEnv,
    });
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.DOYA_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.DOYA_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.DOYA_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.DOYA_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
