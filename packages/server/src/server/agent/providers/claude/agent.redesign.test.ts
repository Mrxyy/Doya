...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "DOYA_ENV_SENTINEL_SECRET";
  const runtimeSecret = "DOYA_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "DOYA_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.DOYA_TEST_SENTINEL_SECRET;
  process.env.DOYA_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        DOYA_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.DOYA_TEST_SENTINEL_SECRET;
    } else {
      process.env.DOYA_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
