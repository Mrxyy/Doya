...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.doya.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.doya.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(path.join(os.tmpdir(), "doya-offer-home-"));

    const { logger: logger1 } = createCapturingLogger();
    const daemon1 = await createTestDoyaDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      doyaHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestDoyaDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        doyaHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(path.join(os.tmpdir(), "doya-offer-e2e-"));
    ...
    const env = {
      ...process.env,
      DOYA_HOME: tempHome,
      DOYA_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      DOYA_DICTATION_ENABLED: "0",
      DOYA_VOICE_MODE_ENABLED: "0",
      DOYA_LOG_FORMAT: "json",
    };
