...
import { createTestDoyaDaemon, type TestDoyaDaemon } from "../test-utils/doya-daemon.js";
import {
...
const cleanupDaemons = new Set<TestDoyaDaemon>();
...
test("openProject reclassifies an existing directory workspace into its parent git project", async () => {
  const previousSupervised = process.env.DOYA_SUPERVISED;
  process.env.DOYA_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "doya-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "doya-open-project-worktree-")),
    );
    const doyaHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "doya-open-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(worktreeRoot);
    cleanupPaths.add(doyaHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@getdoya.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Doya Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    const projectsPath = path.join(doyaHome, "projects", "projects.json");
    const workspacesPath = path.join(doyaHome, "projects", "workspaces.json");
    ...
    const daemon = await createTestDoyaDaemon({ doyaHomeRoot, cleanup: false });
    ...
    process.env.DOYA_SUPERVISED = previousSupervised;
  }
}, 30_000);
