...
import {
  markDoyaWorktreeFirstAgentBranchAutoNameAttempted,
  readDoyaWorktreeMetadata,
  writeDoyaWorktreeFirstAgentBranchAutoNameMetadata,
} from "../utils/worktree-metadata.js";
import type { WorktreeCreationIntent } from "./resolve-worktree-creation-intent.js";
import { buildAgentBranchNameSeed } from "./agent/prompt-attachments.js";
import type { FirstAgentContext } from "@getdoya/protocol/messages";

export interface CreateDoyaWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
}

export interface CreateDoyaWorktreeResult {
...
export type CreateDoyaWorktreeFn = (
...
export interface CreateDoyaWorktreeDeps extends CreateWorktreeCoreDeps {
  projectRegistry: Pick<ProjectRegistry, "get" | "upsert">;
  workspaceRegistry: Pick<WorkspaceRegistry, "get" | "list" | "upsert">;
  workspaceGitService: WorkspaceGitService;
}

export async function createDoyaWorktree(
  input: CreateDoyaWorktreeInput,
  deps: CreateDoyaWorktreeDeps,
): Promise<CreateDoyaWorktreeResult> {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readDoyaWorktreeMetadata>;
  try {
    metadata = readDoyaWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markDoyaWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeDoyaWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
...
async function upsertWorkspaceForWorktree(options: {
  ...
  repoRoot: string;
  worktree: WorktreeConfig;
  deps: Pick<CreateDoyaWorktreeDeps, "projectRegistry" | "workspaceRegistry">;
