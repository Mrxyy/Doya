...
DOYA_HOME=~/.doya        # Runtime state directory (agents/, etc.)
DOYA_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`DOYA_HOME` defaults to `~/.doya` and isolates runtime artifacts like `agents/`. `DOYA_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
DOYA_HOME=~/.doya-blue DOYA_LISTEN=127.0.0.1:7777 npm run dev
```
