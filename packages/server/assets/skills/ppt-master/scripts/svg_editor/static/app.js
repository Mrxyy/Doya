...
(function () {
  ...
  var MESSAGES = {
    en: {
      ...
      warn_svg_no_dims:
        "SVG is missing width/height attributes. Please ask the AI to strictly follow shared-standards.md §4 and include width & height in the SVG root element.",
      ...
      modal_confirm_submit:
        "Apply staged attribute edits and annotations to disk?\n\nThe preview service will keep running.",
      modal_success_submit:
        "Changes saved to svg_output.\n\nReturn to the chat to re-export the PPTX or apply AI-needed annotations. The preview service is still running.",
      modal_confirm_exit:
        "Save staged edits and annotations, then ask the AI to apply the annotations and re-export the PPTX?",
    ...
    zh: {
      ...
      warn_svg_no_dims:
        "SVG 缺少 width/height 属性，预览可能异常。请让 AI 严格遵守 shared-standards.md §4 规范，在 SVG 根元素中补全 width 和 height。",
  ...
  window.addEventListener("message", function (event) {
    ...
    if (
      message.source === "doya" &&
      message.type === "doya:ppt-preview:apply-annotations-complete"
    ) {
  ...
  function loadSlides() {
    return fetch("/api/slides")
      ...
      .then(function (data) {
        ...
        slideNames = slides.map(function (s) {
          return s.name;
        });
  ...
  function setupSvgInteraction() {
    ...
    svg.addEventListener("click", function (e) {
      ...
      var picked = e.altKey && target.closest ? target.closest("g[id]") : null;
      picked =
        picked || (target.closest && target.closest("[data-icon][id]")) || target.closest("[id]");
  ...
  function updateSelectionPanel() {
    ...
    if (count === 1) {
      ...
      selectedElementEl.innerHTML =
        ...
        escapeHtml(t("multi_selected", { count: count })) +
        "</span>";
  ...
  function beginDrag() {
    ...
    dragEls.forEach(function (el) {
      ...
      try {
        dragStartAttrs[el.id] = attrsForMove(el, 0, 0);
      } catch (err) {
        dragStartAttrs[el.id] = null;
      }
    });
      ...
      el.setAttribute(
        ...
        "matrix(" +
          [
            ...
            formatCoord(m.c),
            formatCoord(m.d),
            formatCoord(parseFloat(attrs.x)),
  ...
  function updateDragPreview(dxScreen, dyScreen) {
    dragEls.forEach(function (el) {
      applyMoveLocal(el, dxScreen, dyScreen);
    });
  ...
  function commitDrag(dxScreen, dyScreen) {
    ...
    dragEls.forEach(function (el) {
      ...
      stageEditRequest(el.id, payload)
        .then(function () {
  ...
  function sendNudge(key, el, single, slide, payload) {
    ...
    stageEditRequest(el.id, payload, slide)
      .then(function () {
        ...
        if (q) {
          ...
          sendNudge(key, q.el, q.single, q.slide, q.payload);
        }
      })
      .catch(function (err) {
        nudgeFlight[key] = false;
  ...
  function nudgeSelection(dxScreen, dyScreen) {
    ...
    els.forEach(function (el) {
      ...
      if (!nudgeFlight[key] && !nudgeQueued[key]) {
        ...
        try {
          nudgeLastOk[key] = attrsForMove(el, 0, 0);
        } catch (err) {
          delete nudgeLastOk[key];
        }
      }
      var d = screenDeltaToLocal(el, dxScreen, dyScreen);
      var attrs;
  ...
  function collectSelectableAt(clientX, clientY) {
    ...
    document.elementsFromPoint(clientX, clientY).forEach(function (node) {
      ...
      var picked =
        (node.closest && node.closest("[data-icon][id]")) || (node.closest && node.closest("[id]"));
  ...
  function showOverlapPicker(clientX, clientY, candidates) {
    ...
    candidates.forEach(function (el) {
      ...
      var tag = el.tagName.toLowerCase();
      var txt = tag === "text" || tag === "tspan" ? (el.textContent || "").trim().slice(0, 28) : "";
      item.innerHTML =
        '<span class="overlap-tag">&lt;' +
        escapeHtml(tag) +
        '&gt;</span><span class="overlap-id">' +
        escapeHtml(el.id) +
        "</span>" +
        (txt ? '<span class="overlap-txt">' + escapeHtml(txt) + "</span>" : "");
      item.addEventListener("mouseenter", function () {
        el.classList.add("overlap-hover");
      });
      item.addEventListener("mouseleave", function () {
        el.classList.remove("overlap-hover");
      });
  ...
  function selectByRubberBand(screenRect) {
    ...
    selectableEls.forEach(function (el) {
      try {
        ...
        // AABB intersection
        if (
          minX < screenRect.right &&
          maxX > screenRect.left &&
          minY < screenRect.bottom &&
          maxY > screenRect.top
        ) {
          var eid = el.id;
          if (eid) {
  ...
  function initSlideNav() {
    if (navFirstBtn)
      navFirstBtn.addEventListener("click", function () {
        gotoSlideIndex(0);
      });
    if (navPrevBtn)
    ...
    if (navLastBtn)
      navLastBtn.addEventListener("click", function () {
        gotoSlideIndex(slideNames.length - 1);
      });
  }
  ...
  function updateAnnotationList() {
    ...
    if (ids.length === 0) {
      annotationsEl.innerHTML =
        '<div class="annotations-empty">' + escapeHtml(t("empty_annotations")) + "</div>";
      return;
    }
  ...
  modalConfirm.addEventListener("click", function () {
    if (pendingModalAction === "exit") {
      ...
      drainDirectEdits()
        .then(function () {
          return fetch("/api/save-all", { method: "POST" });
        })
        .then(function (res) {
          return res.json();
        })
        .then(function (data) {
          ...
          updateUndoButton();
          if (currentSlide) {
            var item = slideListEl.querySelector(
              '.slide-item[data-name="' + cssAttr(currentSlide) + '"]',
            );
            selectSlide(currentSlide, item || undefined);
          }
      ...
      .then(function (data) {
        if (data.error) {
          ...
          loadSlides().then(function () {
            if (activeSlide) {
              var item = slideListEl.querySelector(
                '.slide-item[data-name="' + cssAttr(activeSlide) + '"]',
              );
  ...
  function sanitizeSvg(svgString) {
    ...
    doc.querySelectorAll("script,foreignObject").forEach(function (el) {
      el.remove();
    });
  ...
  function showError(msg) {
    ...
    banner.style.cssText =
      "position:fixed;top:0;left:0;right:0;padding:10px 16px;background:#ef4444;color:#fff;font-size:13px;text-align:center;z-index:999;cursor:pointer;";
  ...
  function showWarning(msg) {
    ...
    var banner = document.createElement("div");
    banner.style.cssText =
      "position:fixed;top:38px;left:0;right:0;padding:8px 16px;background:#f59e0b;color:#1f1300;font-size:12px;text-align:center;z-index:998;cursor:pointer;";
    banner.textContent = msg;
    banner.onclick = function () {
      banner.remove();
    };
  ...
  function showReloadBanner(name) {
    ...
    banner.style.cssText =
      "position:fixed;top:0;left:0;right:0;padding:10px 16px;background:#2563eb;color:#fff;font-size:13px;text-align:center;z-index:1000;cursor:pointer;";
  ...
  function renderEditableProps(el) {
    ...
    var html =
      ...
      escapeHtml(isGroup ? t("label_group_edit") : t("label_direct_edit")) +
      "</div>";

    var parentGroup = nearestParentGroup(el);
    if (parentGroup && !isGroup) {
      html +=
        '<table class="prop-table"><tr><td class="prop-key">group</td><td class="prop-val">' +
        '<button type="button" class="btn-select-group" data-group-id="' +
        escapeHtml(parentGroup.id) +
        '">' +
        escapeHtml(t("btn_select_group")) +
        "</button></td></tr></table>";
    }
    ...
    try {
      ...
      html +=
        '<tr><td class="prop-key">position</td><td class="prop-val geo-cell">' +
        '<input type="number" class="prop-edit prop-edit-geo" data-geo="x" value="' +
        Math.round(rb.x) +
        '">' +
        '<input type="number" class="prop-edit prop-edit-geo" data-geo="y" value="' +
        Math.round(rb.y) +
        '">' +
        "</td></tr>";
      html +=
        '<tr><td class="prop-key">size</td><td class="prop-val geo-cell">' +
        '<input type="number" class="prop-edit prop-edit-geo" data-geo="w" min="1" value="' +
        Math.round(rb.w) +
        '">' +
        '<input type="number" class="prop-edit prop-edit-geo" data-geo="h" min="1" value="' +
        Math.round(rb.h) +
        '">' +
        "</td></tr>";
    } catch (e) {
    ...
    if (tag === "text" || tag === "tspan") {
      ...
      if (el.querySelector("tspan")) {
        html +=
          '<tr><td class="prop-key">content</td><td class="prop-val prop-note">' +
          escapeHtml(t("prop_multiline_hint")) +
          "</td></tr>";
      } else {
        html +=
          '<tr><td class="prop-key">content</td><td class="prop-val">' +
  ...
  function isColorAttribute(key, value) {
    var k = String(key).toLowerCase();
    if (
      ["fill", "stroke", "color", "stop-color", "flood-color", "lighting-color"].indexOf(k) !== -1
    ) {
  ...
  function renderAttributeControl(spec) {
    if (isColorAttribute(spec.key, spec.value)) {
      return (
        '<input type="color" class="prop-edit prop-edit-color" data-key="' +
        escapeHtml(spec.key) +
        '" value="' +
        normalizeHexForPicker(spec.value) +
        '">' +
    ...
    if (
      spec.value.length > 60 ||
      spec.key === "d" ||
      spec.key === "points" ||
      spec.key === "style"
    ) {
      return (
        '<textarea class="prop-edit prop-edit-attr-area" rows="2" data-key="' +
        escapeHtml(spec.key) +
        '">' +
        escapeHtml(spec.value) +
        "</textarea>"
    ...
    return (
      '<input type="text" class="prop-edit prop-edit-attr" data-key="' +
      escapeHtml(spec.key) +
      '" value="' +
      escapeHtml(spec.value) +
      '">'
  ...
  function tspanPromotionPayload(tspan) {
    ...
    if (!close(own.a, 1) || !close(own.b, 0) || !close(own.c, 0) || !close(own.d, 1)) return null;
    return {
  ...
  function promoteTspanDom(tspan, promoted) {
    ...
    if (next && nextBase && !next.hasAttribute("y") && next.hasAttribute("dy")) {
      next.setAttribute(
        "dy",
        adjustedDyValue(next.getAttribute("dy"), nextBase.y, prevBase ? prevBase.y : parentBaseY),
      );
    }
  ...
  function rawMoveAttrs(el, dx, dy) {
    if (
      localName(el) === "g" &&
      el.hasAttribute("data-icon") &&
      el.hasAttribute("data-use-x") &&
      el.hasAttribute("data-use-y")
    ) {
    ...
    var bakeBox = function () {
      ...
      if (!x || !y || !w || !h || x.unit !== y.unit || x.unit !== w.unit || x.unit !== h.unit)
        return false;
  ...
  function attachPropEditors(el) {
    ...
    if (panel.querySelector(".prop-edit-geo")) {
      ...
      panel
        .querySelectorAll('.prop-edit-geo[data-geo="x"], .prop-edit-geo[data-geo="y"]')
      ...
      panel
        .querySelectorAll('.prop-edit-geo[data-geo="w"], .prop-edit-geo[data-geo="h"]')
        .forEach(function (inp) {
          inp.addEventListener("change", function () {
            commitGeo("resize");
          });
  ...
  function renderMultiSelectSummary(ids) {
    ...
    if (moveEls.length > 0) {
      try {
        ...
        summary +=
          '<table class="prop-table"><tr><td class="prop-key">x / y</td>' +
  ...
  function renderBatchControl(spec) {
    ...
    if (spec.type === "color") {
      return (
        '<input type="color" class="prop-edit prop-edit-batch-color" data-key="' +
        escapeHtml(spec.key) +
        '" value="' +
        normalizeHexForPicker(spec.value) +
        '">' +
        ...
        '" value="' +
        escapeHtml(spec.mixed ? "" : spec.value) +
        '">'
      );
    }
    if (spec.type === "number") {
      return (
        '<input type="number" class="prop-edit prop-edit-batch" data-key="' +
        escapeHtml(spec.key) +
        '" min="' +
        escapeHtml(spec.min || "") +
        '" max="' +
    ...
    if (spec.type === "select") {
      var html =
        '<select class="prop-edit prop-edit-batch" data-key="' + escapeHtml(spec.key) + '">';
