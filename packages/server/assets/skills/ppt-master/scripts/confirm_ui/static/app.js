 ...
 * AI source, formula policy, animation, generation mode) list ALL options from
...
(function () {
  ...
  var MESSAGES = {
    en: {
      ...
      topbar_hint_embedded:
        "Pick or type your choices, then click Confirm. Doya will continue the agent automatically.",
      loading: "Loading…",
      ...
      animation_subtle_desc: "AI applies restrained semantic builds to the final PPTX.",
      animation_presenter_reveal_desc: "Click-paced reveals for live talks.",
      animation_showcase_desc: "More motion for launch, brand, and roadshow decks.",
      animation_page_only_desc: "Slide transitions only; no per-element builds.",
      animation_none_desc: "No transitions or element animation.",
    ...
    zh: {
      ...
      topbar_hint_embedded: "选择或自定义各项后点「确认」；豆芽会自动让 Agent 继续。",
      loading: "加载中…",
      ...
      animation_subtle_desc: "由 AI 为最终 PPTX 添加克制的语义分组动画。",
      animation_presenter_reveal_desc: "适合现场演讲的点击逐步揭示。",
      animation_showcase_desc: "面向发布、品牌、路演的更强动效节奏。",
      animation_page_only_desc: "仅保留页面切换，不做页内元素动画。",
      animation_none_desc: "关闭页面切换和页内动画。",
      font_heading: "标题",
      font_body: "正文",
      ...
      custom_color: "自定义配色",
      custom_color_placeholder:
        "用文字描述配色，如：深蓝主色、暖橙强调、白色背景——或直接粘贴 HEX 值…",
      role_background: "背景",
  ...
  if (EMBEDDED_IN_DOYA) {
    document.body.classList.add("embedded-doya");
  }
  var URL_LANG = (function () {
    var value = queryParam("lang");
  ...
  function applyStaticTranslations() {
    ...
    if (EMBEDDED_IN_DOYA) {
      ...
      if (hint) hint.textContent = t("topbar_hint_embedded");
    }
  }
  ...
  function legacyRecId(field) {
    ...
    if (field === "animation") return REC.animation && REC.animation.value;
  ...
  function buildCanvasOptionVisual(option) {
    var id = option && option.id ? option.id : "custom";
    var visual = el("span", "canvas-option-visual canvas-option-" + id);
    visual.appendChild(el("span", "option-sprite option-sprite-canvas"));
    return visual;
  }

  function buildRichOptionVisual(kind, option) {
    var id = option && option.id ? option.id : "custom";
    var visual = el(
      "span",
      "rich-option-visual rich-option-" + kind + " rich-option-" + kind + "-" + id,
    );
    visual.appendChild(el("span", "rich-option-panel"));
    visual.appendChild(el("span", "rich-option-line rich-option-line-main"));
    visual.appendChild(el("span", "rich-option-line rich-option-line-soft"));
    visual.appendChild(el("span", "rich-option-mark rich-option-mark-a"));
    visual.appendChild(el("span", "rich-option-mark rich-option-mark-b"));
    visual.appendChild(el("span", "rich-option-mark rich-option-mark-c"));
    return visual;
  }

  function buildFieldVisual(kind) {
    var visual = el("div", "field-visual field-visual-" + kind);
    visual.appendChild(el("span", "field-visual-panel"));
    visual.appendChild(el("span", "field-visual-line field-visual-line-main"));
    visual.appendChild(el("span", "field-visual-line field-visual-line-soft"));
    visual.appendChild(el("span", "field-visual-mark field-visual-mark-a"));
    visual.appendChild(el("span", "field-visual-mark field-visual-mark-b"));
    visual.appendChild(el("span", "field-visual-mark field-visual-mark-c"));
    return visual;
  }
  ...
  function enumField(parent, list, recommendedId, getVal, setVal, opts2) {
    ...
    if (allowCustom) {
      if (grouped) {
        list = list.map(function (g) {
          var items = (g.items || []).filter(function (o) {
            return o.id !== "custom";
          });
          var copy = {};
        ...
        flat = list.reduce(function (a, g) {
          return a.concat(g.items || []);
        }, []);
      } else {
        flat = flat.filter(function (o) {
          return o.id !== "custom";
        });
      }
    }
    var ids = flat.map(function (o) {
      return o.id;
    });
    ...
    function makeChip(o) {
      ...
      if (desc && !opts2.optionVisual) label += (LANG === "zh" ? "：" : " — ") + desc;
      var chip = el("div", "chip");
      if (desc) chip.title = desc;
      if (opts2.optionVisual === "canvas") {
        chip.classList.add("chip-canvas-option");
        chip.appendChild(buildCanvasOptionVisual(o));
        chip.appendChild(el("span", "chip-text canvas-option-text", label));
      } else if (opts2.optionVisual) {
        chip.classList.add("chip-rich-option");
        chip.appendChild(buildRichOptionVisual(opts2.optionVisual, o));
        chip.appendChild(el("span", "chip-text rich-option-text", label));
      } else {
    ...
    if (grouped) {
      list.forEach(function (g) {
        ...
        if (opts2.optionVisual) row.classList.add("rich-option-grid");
        (g.items || []).forEach(function (o) {
          row.appendChild(makeChip(o));
        });
        parent.appendChild(row);
      });
      if (allowCustom) {
        var lastRow = el("div", "chips");
        if (opts2.optionVisual) lastRow.classList.add("rich-option-grid");
        lastRow.appendChild(buildCustomChip());
        parent.appendChild(lastRow);
      }
    } else {
      var wrap = el("div", "chips");
      if (opts2.optionVisual === "canvas") wrap.classList.add("canvas-option-grid");
      else if (opts2.optionVisual) wrap.classList.add("rich-option-grid");
      flat.forEach(function (o) {
        wrap.appendChild(makeChip(o));
      });
    ...
    function buildCustomChip() {
      ...
      if (opts2.optionVisual === "canvas") {
        customChip.classList.add("chip-canvas-option", "chip-canvas-custom");
        customChip.textContent = "";
        customChip.appendChild(buildCanvasOptionVisual({ id: "custom" }));
        customChip.appendChild(el("span", "chip-text canvas-option-text", t("custom")));
      } else if (opts2.optionVisual) {
        customChip.classList.add("chip-rich-option", "chip-rich-custom");
        customChip.textContent = "";
        customChip.appendChild(buildRichOptionVisual(opts2.optionVisual, { id: "custom" }));
        customChip.appendChild(el("span", "chip-text rich-option-text", t("custom")));
      }
  ...
  function usesCustomImagePlanValue(value) {
    var ids = (CAT.image_usage || []).map(function (item) {
      return item.id;
    });
  ...
  function renderCanvas(host) {
    ...
    sec.classList.add("section-option-gallery");
    enumField(
      sec,
      CAT.canvas,
      recOrFirst("canvas", CAT.canvas),
      function () {
        return STATE.canvas;
      },
      function (v) {
      ...
      { allowCustom: true, optionVisual: "canvas" },
    );
  ...
  function renderStyle(host) {
    ...
    enumField(
      ...
      { allowCustom: true, optionVisual: "mode" },
    );
    ...
    enumField(
      sub2,
      CAT.visual_styles,
      recOrFirst("visual_style", CAT.visual_styles),
      function () {
        return STATE.visual_style;
      },
      function (v) {
  ...
  function canvasHeight(canvasVal) {
    var dim = null;
    (CAT.canvas || []).forEach(function (o) {
      if (o.id === canvasVal) dim = o.dim;
    });
  ...
  function renderColor(host) {
    var colorSpec = REC.color || REC.colors || {};
    var cands = colorSpec.candidates || [];
    ...
    function selectCard(idx) {
      ...
      grid.querySelectorAll(".color-card").forEach(function (card, i) {
    ...
    function selectCustomColor() {
      ...
      grid.querySelectorAll(".color-card").forEach(function (card) {
    ...
    cands.forEach(function (c, idx) {
      ...
      card.appendChild(sw);
      card.appendChild(
        el("div", "color-name", localized(c, "name") || t("option_prefix") + " " + (idx + 1)),
      );
    ...
    PALETTE_ROLES.forEach(function (role) {
      ...
      inp.addEventListener("input", function () {
        ...
        if (
          n &&
          selectedIdx >= 0 &&
          cardSwatchRefs[selectedIdx] &&
          cardSwatchRefs[selectedIdx][role]
        ) {
    ...
    if (STATE.color && STATE.color.name && STATE.color.name !== "custom") {
      cands.forEach(function (c, i) {
        if (c.name === STATE.color.name) selIdx = i;
      });
    }
  ...
  function renderIcons(host) {
    ...
    enumField(
      ...
      { allowCustom: true, optionVisual: "icons" },
    );
    host.appendChild(sec);
  }
  ...
  function renderTypography(host) {
    ...
    function selectFont(idx) {
      ...
      customInput.style.display = "none";
      grid.querySelectorAll(".font-card").forEach(function (card, i) {
        card.classList.toggle("selected", i === idx);
      });
      refreshStylePreview();
    }
    ...
    cands.forEach(function (c, idx) {
      ...
      top.appendChild(
        el("span", "font-card-name", localized(c, "name") || t("option_prefix") + " " + (idx + 1)),
      );
      var meta =
        t("font_heading") +
        " " +
      ...
      var hbox = el("div", "font-sample-heading-box");
      fontSample(hbox, head, head.css);
      card.appendChild(hbox);
      var bbox = el("div", "font-sample-body-box");
      fontSample(bbox, body, body.css);
    ...
    enumField(
      subfp,
      CAT.formula_policy,
      recOrFirst("formula_policy", CAT.formula_policy),
      function () {
        return STATE.formula_policy;
      },
      function (v) {
        STATE.formula_policy = v;
      },
    );
    ...
    var selIdx = -1;
    if (STATE.typography && STATE.typography.name)
      cands.forEach(function (c, i) {
        if (c.name === STATE.typography.name) selIdx = i;
      });
  ...
  function renderStylePreview(host) {
    ...
    title.appendChild(titleCjk);
    title.appendChild(titleLat);
    ...
    bodyWrap.appendChild(bodyCjk);
    bodyWrap.appendChild(bodyLat);
    bodyRow.appendChild(accentBar);
    bodyRow.appendChild(bodyWrap);
    textcol.appendChild(title);
    textcol.appendChild(bodyRow);
  ...
  function renderImages(host) {
    ...
    function selectImageStrategy(idx) {
      ...
      strategyGrid.querySelectorAll(".font-card").forEach(function (card, i) {
        card.classList.toggle("selected", i === idx);
      });
    }
    strategyCands.forEach(function (c, idx) {
      var card = el("div", "font-card image-strategy-card");
      var top = el("div", "font-card-head");
      top.appendChild(
        el("span", "font-card-name", localized(c, "name") || t("option_prefix") + " " + (idx + 1)),
      );
      ...
      card.addEventListener("click", function () {
        selectImageStrategy(idx);
      });
      strategyGrid.appendChild(card);
    });
    if (!strategyCands.length)
      strategyGrid.appendChild(el("div", "toggle-desc", t("image_strategy_empty")));
    strategySub.appendChild(strategyGrid);
    enumField(
      sec,
      CAT.image_usage,
      recOrFirst("image_usage", CAT.image_usage),
      function () {
        return STATE.image_usage;
      },
      function (v) {
        STATE.image_usage = v;
        refreshAiControls();
    ...
    enumField(
      sub,
      CAT.image_ai_path,
      recOrFirst("image_ai_path", CAT.image_ai_path),
      function () {
        return STATE.image_ai_path;
      },
      function (v) {
        STATE.image_ai_path = v;
      },
      { optionVisual: "source" },
    );
    sec.appendChild(sub);
  ...
  function renderAnimation(host) {
    var sec = section(9, "sec_animation");
    var noteById = {
      subtle: "animation_subtle_desc",
      presenter_reveal: "animation_presenter_reveal_desc",
      showcase: "animation_showcase_desc",
      page_only: "animation_page_only_desc",
      none: "animation_none_desc",
    };
    function refresh() {
      setSectionNote(sec, t(noteById[STATE.animation] || "animation_subtle_desc"));
    }
    enumField(
      sec,
      CAT.animation,
      recOrFirst("animation", CAT.animation),
      function () {
  ...
  function renderMode(host) {
    ...
    function refresh() {
      setSectionNote(
        sec,
        STATE.generation_mode === "split" ? t("mode_split_desc") : t("mode_continuous_desc"),
      );
    }
    enumField(
      sec,
      CAT.generation_mode,
      recOrFirst("generation_mode", CAT.generation_mode),
      function () {
        return STATE.generation_mode;
      },
      function (v) {
  ...
  function renderRefine(host) {
    ...
    enumField(
      sec,
      opts,
      STATE.refine_spec ? "on" : "off",
      function () {
        return STATE.refine_spec ? "on" : "off";
      },
      function (v) {
  ...
  function initState() {
    ...
    STATE.page_count =
      REC.page_count && REC.page_count.value != null ? String(REC.page_count.value) : "";
    ...
    STATE.animation = pick("animation", CAT.animation);

    STATE.generation_mode = pick("generation_mode", CAT.generation_mode);
    STATE.refine_spec = !!(
      (REC.refine_spec && REC.refine_spec.value) ||
      (REC.recommend && REC.recommend.refine_spec)
    );
    applyConfirmedResult();
  }

  function applyConfirmedResult() {
    if (!REC || !REC._confirmed_result || typeof REC._confirmed_result !== "object") return;
    var result = REC._confirmed_result;
    [
      "canvas",
      "page_count",
      "audience",
  ...
  function setConfirmedReadonly(isConfirmed) {
    document.body.classList.toggle("confirmed", Boolean(isConfirmed));
    var actionbar = document.getElementById("actionbar");
    var btn = document.getElementById("btn-confirm");
    if (!actionbar) return;
    actionbar.hidden = Boolean(isConfirmed);
    actionbar.style.display = isConfirmed ? "none" : "flex";
    if (btn) {
      btn.disabled = Boolean(isConfirmed);
    }
  }

  // ---- confirm + close -------------------------------------------------
  function showConfirmedOverlay() {
    if (EMBEDDED_IN_DOYA) return;
  ...
  function loadCatalogs() {
    return fetch("/api/catalogs")
      ...
      .catch(function () {
        return fetch("/static/catalogs.json").then(function (r) {
          return r.json();
        });
      });
  }
  ...
  function boot() {
    ...
    Promise.all([
      loadCatalogs(),
      fetch("/api/recommendations").then(function (r) {
      ...
      .then(function (res) {
        CAT = res[0];
        REC = res[1];
        if (!URL_LANG && (REC.lang === "zh" || REC.lang === "en")) {
          var hasStored = false;
          try {
