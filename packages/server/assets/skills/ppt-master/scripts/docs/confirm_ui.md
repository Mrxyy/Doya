...
- **Closed enumerable** — formula policy / animation / generation mode / refine spec, plus AI source only when image usage may include `ai`. These have no Custom box; out-of-catalog values snap back to the recommended option. Use pipeline vocabulary: icon ids are actual library ids such as `tabler-outline`, or `emoji` for system emoji; image usage labels mirror Strategist terminology: `ai` = AI-generated, `web` = Web-sourced, `provided` = User-provided, `placeholder` = Placeholder, `none` = No images. Animation ids mirror export strategy: `subtle`, `presenter_reveal`, `showcase`, `page_only`, `none`. Use custom prose only when several sources are mixed.
...
**Custom box** appears only on fields whose universe is genuinely open — `canvas`, `mode`, `visual_style`, `icons`, and `image_usage`. Fully closed sets — `image_ai_path`, `formula_policy`, `animation`, `generation_mode`, `refine_spec` — have **no** Custom box; an out-of-catalog value there is snapped back to the recommended option.
    ...
    "candidates": [
      {
        ...
        "heading": {
          "cjk": "思源黑体",
          "latin": "Inter",
          "css": "'Source Han Sans SC','Inter',sans-serif",
          "sample_cjk": "标题示例",
...
- `recommend.animation` is the Strategist's motion recommendation. Use `subtle` for most polished generated decks; `presenter_reveal` for live-talk click pacing; `showcase` for brand, launch, roadshow, or event decks; `page_only` for formal, compliance-heavy, or print-oriented decks; `none` when the user asks for static output. The confirmed value controls Step 7.3 export flags.
...
- `animation` is always written and must be honored at export: `subtle` → `-a auto --animation-trigger after-previous`; `presenter_reveal` → `-a auto --animation-trigger on-click`; `showcase` → `-a mixed --animation-trigger after-previous --animation-stagger 0.35`; `page_only` → no `-a` flag, keep the default page transition; `none` → `-t none -a none`.
