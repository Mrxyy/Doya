...
import { DoyaConfigRawSchema, DoyaConfigSchema } from "@getdoya/protocol/doya-config-schema";

describe("Doya config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = DoyaConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(DoyaConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
