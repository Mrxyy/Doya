...
$RemoteDebuggingPort = if ($env:DOYA_ELECTRON_REMOTE_DEBUGGING_PORT) {
    $env:DOYA_ELECTRON_REMOTE_DEBUGGING_PORT
} else {
    "9223"
}
$ExistingElectronFlags = if ($env:DOYA_ELECTRON_FLAGS) {
    "$($env:DOYA_ELECTRON_FLAGS) "
} else {
    ""
}
$env:DOYA_ELECTRON_FLAGS = "$($ExistingElectronFlags)--remote-debugging-port=$RemoteDebuggingPort"
...
& "$RootDir\node_modules\.bin\concurrently" `
    ...
    "cd `"$AppDir`" && `$env:DOYA_WEB_PLATFORM = `"electron`"; npx expo start --port $($env:EXPO_PORT)" `
