...
function getCliShimPath(appPath) {
  if (process.platform === "darwin") {
    return path.join(appPath, "Contents", "Resources", "bin", "doya");
  }

  if (process.platform === "win32") {
    return path.join(appPath, "resources", "bin", "doya.cmd");
  }

  return path.join(appPath, "resources", "bin", "doya");
}
...
function createDefaultDaemonEnv(extraEnv) {
  ...
  delete env.DOYA_HOME;
  delete env.DOYA_LISTEN;
  return env;
}

function parseSmokeLine(line) {
  const prefix = "[doya-smoke] ";
...
function formatLogs({ stdout, stderr, userData }) {
  ...
  const daemonLog = readIfExists(path.join(os.homedir(), ".doya", "daemon.log"));
  return [
...
async function smokeCliTerminal({ appPath, env }) {
  const cwd = createTempDir("doya-smoke-terminal-cwd-");
  const marker = `doya-packaged-terminal-smoke-${Date.now()}`;
...
async function smokePackagedDesktopApp({ appPath }) {
  ...
  const userData = createTempDir("doya-smoke-user-data-");
  const env = createDefaultDaemonEnv({
    DOYA_DESKTOP_SMOKE: "1",
    DOYA_ELECTRON_USER_DATA_DIR: userData,
  });
...
if (require.main === module) {
  ...
  if (!appPath) {
    process.stderr.write("Usage: node smoke-packaged-desktop-app.js --app <Doya.app>\n");
    process.exit(2);
  }
