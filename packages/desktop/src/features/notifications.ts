...
export function registerNotificationHandlers(): void {
  ipcMain.handle("doya:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("doya:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("doya:event:notification-click", payload);
      }
