...
const originalDesktopCli = process.env.DOYA_DESKTOP_CLI;
...
describe("passthrough CLI", () => {
  afterEach(() => {
    ...
    if (originalDesktopCli === undefined) {
      delete process.env.DOYA_DESKTOP_CLI;
    } else {
      process.env.DOYA_DESKTOP_CLI = originalDesktopCli;
    }
  });
  ...
  it("returns null when no CLI args are provided", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Doya.app/Contents/MacOS/Doya"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores macOS GUI launch arguments", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Doya.app/Contents/MacOS/Doya", "-psn_0_12345"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toBeNull();
  ...
  it("ignores --no-sandbox injected by Linux wrapper", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/usr/bin/Doya", "--no-sandbox", "status"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["status"]);
  ...
  it("preserves CLI flags for direct app invocations", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Doya.app/Contents/MacOS/Doya", "--version"],
        isDefaultApp: false,
        forceCli: false,
      }),
    ).toEqual(["--version"]);
  ...
  it("passes --open-project through as a normal CLI arg", () => {
    expect(
      parsePassthroughCliArgs({
        argv: ["/Applications/Doya.app/Contents/MacOS/Doya", "--open-project", "/tmp/project"],
        isDefaultApp: false,
        forceCli: false,
      }),
  ...
  it("parses terminal args for direct app CLI passthrough", () => {
    ...
    expect(
      parsePassthroughCliArgsFromArgv([
        "/Applications/Doya.app/Contents/MacOS/Doya",
        "daemon",
        "set-password",
      ]),
