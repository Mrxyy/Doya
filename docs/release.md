...
```jsonc
// mcp__doya__create_heartbeat arguments
{
...
- `version:all:*` bumps root + syncs workspace versions and `@getdoya/*` dependency versions
...
Format: append `([#123](https://github.com/getdoya/doya/pull/123) by [@user](https://github.com/user))` at the end of each bullet. For changes spanning multiple PRs or contributors:

```markdown
- Voice mode now works on tablets with proper microphone permissions. ([#210](https://github.com/getdoya/doya/pull/210), [#215](https://github.com/getdoya/doya/pull/215) by [@alice](https://github.com/alice), [@bob](https://github.com/bob))
```

Rules:

- **Always link the PR number** as `[#N](https://github.com/getdoya/doya/pull/N)`.
