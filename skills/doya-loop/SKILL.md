...
Read the **doya** skill. Before choosing worker or verifier providers, read `~/.doya/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not start the loop until you have read it.

Loops are a CLI primitive: `doya loop run`. Manage with `doya loop ls`, `doya loop inspect <id>`, `doya loop logs <id>`, `doya loop stop <id>`.
