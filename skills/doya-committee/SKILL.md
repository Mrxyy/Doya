...
Read the **doya** skill. Before choosing committee members, read `~/.doya/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not create committee agents until you have read it.
...
Create both agents in parallel via Doya with `[Committee] <task>` titles and the same prompt. Wait for both — not just whichever finishes first.
