---
name: doya
description: Doya reference for managing agents and worktrees. Load whenever you need to create agents, send them prompts, or manage worktrees.
---

Doya is a daemon that supervises AI coding agents on your machine. Control it through tools or a CLI.
...
The `doya` CLI is a thin wrapper over the same daemon. Same surface:

```bash
doya run --provider codex/gpt-5.4 --mode full-access --worktree feat/x "<prompt>"
doya send <agent-id> "<follow-up>"
doya ls
doya worktree ls
doya schedule create --cron "*/15 * * * *" "ping main build"
```

Discover with `doya --help` and `doya <cmd> --help`.

**If `doya` isn't on PATH but the desktop app is installed**, the bundled CLI is at:

- macOS: `/Applications/Doya.app/Contents/Resources/bin/doya`
- Linux: `<install-dir>/resources/bin/doya`
- Windows: `C:\Program Files\Doya\resources\bin\doya.cmd`

The desktop app's first-run hook (`installCli`) symlinks this to `~/.local/bin/doya` (macOS/Linux) or drops a `.cmd` trampoline (Windows) and adds `~/.local/bin` to PATH via shell rc files. If that didn't take, offer to symlink it — don't do it silently.
...
|                | Default                                                        |
| -------------- | -------------------------------------------------------------- |
| Listen address | `127.0.0.1:6767` (override with the supported listen env)      |
| Home           | `~/.doya` (legacy `DOYA_HOME` override remains supported)      |
| Daemon log     | `$DOYA_HOME/daemon.log`                                        |
| Agent state    | `$DOYA_HOME/agents/<id>.json`                                  |
| Worktrees      | `$DOYA_HOME/worktrees/` (or `worktrees.root` in `config.json`) |
| PID file       | `$DOYA_HOME/doya.pid`                                          |
| Health         | `GET http://127.0.0.1:6767/api/health`                         |

Debug order:

1. `tail -n 200 ~/.doya/daemon.log`.
2. `doya daemon status` for liveness.
